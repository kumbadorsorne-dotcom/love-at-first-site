...
function LiveCount() {
  ...
  return (
    <div style={{ display:'inline-flex', alignItems:'center', gap: 10, padding:'8px 14px', border:'1px solid #FFFFFF30', borderRadius: 2, background:'#0A0A0A' }}>
      <span style={{ width: 8, height: 8, borderRadius: 99, background:'#800120', animation:'pulse 1.2s infinite' }}/>
      <span className="mono caps" style={{ fontSize: 10, letterSpacing:'0.16em', color:'#FFFFFF' }}>Live now</span>
      <span className="serif" style={{ fontSize: 20, lineHeight: 1, fontFeatureSettings:'"tnum"', color:'#FFFFFF' }}>
        {n.toLocaleString()}
      </span>
      <span className="mono caps" style={{ fontSize: 9, letterSpacing:'0.14em', color:'#888888' }}>in pods</span>
    </div>
  );
}
    ...
    <div style={{ height:'100vh', display:'grid', gridTemplateColumns:'1fr 1fr', overflow:'hidden' }}>
      ...
      <div style={{ position:'relative', padding:'36px 40px 28px', borderRight:'1px solid #FFFFFF15', background:'#1A1A1A', display:'flex', flexDirection:'column', justifyContent:'space-between', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex: 1, display:'flex', flexDirection:'column', alignItems:'flex-start', justifyContent:'center', flex: 1 }}>
          <div className="mono caps" style={{ fontSize: 10, letterSpacing:'0.16em', color:'#888888' }}>Eight First Dates</div>
          <div className="serif" style={{ fontSize: 72, lineHeight: 0.95, letterSpacing:'-0.03em', marginTop: 10, color:'#FFFFFF' }}>
          ...
          <div className="serif" style={{ fontSize: 18, fontStyle:'italic', color:'#CCCCCC', marginTop: 16, lineHeight: 1.3, maxWidth: 440 }}>
            Nine strangers. Eight calls. Five minutes each. No swiping, no edits, no second glances at anyone you've already met.
          </div>
        ...
        <div style={{ position:'relative', zIndex: 1 }}>
          <div className="mono caps" style={{ fontSize: 10, letterSpacing:'0.16em', color:'#888888', marginBottom: 10 }}>How It Works</div>
          {[
            'Sign up and tell us about yourself.',
            'Enter the lobby and wait for your pod.',
            'Eight 5-minute video calls, back to back.',
            'Vote yes or no after each call.',
            'Matches revealed. Start chatting.'
          ].map((t, i) => (
            <div key={i} style={{ fontSize: 16, lineHeight: 1.35, borderBottom:'1px dotted #FFFFFF20', padding:'7px 0', display:'flex', gap: 14, alignItems:'baseline', color:'#FFFFFF', fontFamily:'Lato, sans-serif' }}>
              <span className="mono" style={{ fontSize: 11, color:'#800120', minWidth: 22 }}>{String(i+1).padStart(2,'0')}</span>
              <span>{t}</span>
            </div>
          ))}
        </div>
      ...
      <div style={{ padding:'28px 40px 20px', display:'flex', flexDirection:'column', justifyContent:'center' }}>
        <div style={{ maxWidth: 420, width:'100%' }}>
          <div style={{ display:'flex', gap: 4, padding: 4, border:'1px solid var(--line-strong)', borderRadius: 2, width:'fit-content', marginBottom: 16 }}>
            {['signup','signin'].map(m => (
              <button key={m} onClick={() => setMode(m)} style={{
                padding:'6px 16px', fontSize: 11, letterSpacing:'0.12em', textTransform:'uppercase', fontFamily:'Lato, sans-serif',
          ...
          <div className="serif" style={{ fontSize: 36, lineHeight: 1.02, letterSpacing:'-0.02em' }}>
            {mode === 'signin'
          ...
          <div className="mono" style={{ fontSize: 12, color:'var(--ink-3)', marginTop: 12, lineHeight: 1.5 }}>
            {mode === 'signin' ? 'The next pod begins the moment you\'re ready.' : 'Takes about ten minutes. Be honest \u2014 every answer is locked after.'}
          </div>
          ...
          <div style={{ marginTop: 16, display:'flex', flexDirection:'column', gap: 10 }}>
            ...
            {mode === 'signup' && (
              <label style={{ display:'flex', alignItems:'flex-start', gap: 10, marginTop: 4 }}>
                ...
                <span style={{ fontSize: 13, lineHeight: 1.3, color:'var(--ink-2)', fontFamily:'Lato, sans-serif' }}>
            ...
            <button onClick={() => window.__setView && window.__setView(mode === 'signin' ? 'dashboard' : 'quiz')} style={{
              ...
              {mode === 'signin' ? 'Start Dating \u2192' : 'Get Started \u2192'}
            </button>

            <div style={{ display:'flex', alignItems:'center', gap: 10, margin:'8px 0 2px' }}>
              <div style={{ flex:1, height:1, background:'var(--line-strong)' }}/>
              <span className="mono caps" style={{ fontSize: 9, color:'var(--ink-3)', letterSpacing:'0.18em' }}>or</span>
              <div style={{ flex:1, height:1, background:'var(--line-strong)' }}/>
            </div>
            <button style={{ padding:'10px 14px', border:'1px solid var(--line-strong)', borderRadius: 2, fontFamily:'Lato, sans-serif', fontSize: 11, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--ink-2)' }}>
              Continue with Apple
            </button>
            <button style={{ padding:'10px 14px', border:'1px solid var(--line-strong)', borderRadius: 2, fontFamily:'Lato, sans-serif', fontSize: 11, letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--ink-2)' }}>
          ...
          <div className="mono" style={{ fontSize: 10, color:'var(--ink-3)', marginTop: 14, fontStyle:'italic', lineHeight: 1.6 }}>
            By continuing you confirm you are at least 18. Camera, microphone, and decorum rules apply during calls.
          </div>
        </div>
      </div>
    </div>
...
function Field({ label, value, onChange, placeholder, type = 'text' }) {
  ...
  return (
    <label style={{ display:'block' }}>
      <div className="caps mono" style={{ fontSize: 9, letterSpacing:'0.16em', color:'var(--ink-3)', marginBottom: 6 }}>{label}</div>
      <input
        ref={ref}
        type={isPw ? 'text' : type}
        ...
        style={{
          width:'100%', padding:'9px 12px', border:'1px solid var(--line-strong)', borderRadius: 2,
          background:'var(--cream)', color:'var(--ink)', fontFamily:'Lato, sans-serif', fontSize: 13,
          outline:'none'
        }}
      />
    </label>
